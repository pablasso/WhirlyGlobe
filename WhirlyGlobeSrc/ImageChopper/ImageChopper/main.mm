...
bool BuildLevel(int outX,int outY,const char *levelId,NSImage *img,int outSize,int borderSize,const char *outDir,const char *outName,OutFormatType outFormatType,const char *texTool)
{
    ...
    for (unsigned int ix=0;ix<outX;ix++)
    {
        ...
        for (unsigned int iy=0;iy<outY;iy++)
        {
            ...
            NSBitmapImageRep *imageRep = [[[NSBitmapImageRep alloc] initWithBitmapDataPlanes:NULL pixelsWide:outSize pixelsHigh:outSize bitsPerSample:8 samplesPerPixel:4 hasAlpha:YES isPlanar:NO colorSpaceName:NSDeviceRGBColorSpace bytesPerRow:4*outSize bitsPerPixel:32] autorelease];
            ...
            {
                char cmd[1024];
                sprintf(cmd,"%s -e PVRTC --channel-weighting-linear --bits-per-pixel-4 -o %s/%s.pvrtc %s/%s.%s",
                        texTool,outDir,imgName,outDir,imgName,[ext UTF8String]);
                if (system(cmd))
                {
